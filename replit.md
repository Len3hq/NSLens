...
- Outbound: `evaluateReminders` and Hub fan-out also send to `telegramChatId` via `sendTelegramMessage` when set. Hub fan-out caps Telegram DMs to the top `MAX_TELEGRAM_FANOUT` (3) most-relevant linked recipients per post — everyone matched still gets the in-app notification, only the strongest matches get pushed. Each Telegram match message includes a deep link `${PUBLIC_APP_URL || REPLIT_DEV_DOMAIN}/app/hub#post-<id>` so the recipient can read the full post inline; Hub.tsx renders each post card with `id="post-<id>"` for the anchor.
